...
namespace Mango.Web.Controllers
{
    public class OrderController : Controller
    {
        ...
        public IActionResult GetAll(string status)
        {
            ...
            {
                list = JsonConvert
                    .DeserializeObject<List<OrderHeaderDto>>(Convert.ToString(response.Result));

                switch (status)
                {
                    case "approved":
                        list = list.Where(od => od.Status == SD.Status_Approved);
                        break;
                    case "readyforpickup":
                        list = list.Where(od => od.Status == SD.Status_ReadyForPickup);
                        break;
                    case "cancelled":
                        list = list.Where(od => od.Status == SD.Status_Cancelled || od.Status == SD.Status_Refunded);
                        break;
                    default:
                        break;
                }
